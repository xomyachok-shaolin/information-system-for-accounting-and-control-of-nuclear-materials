...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from Предприятие_партнер where ИД_предприятия like '+help2+
                                              ' or Наименование like'+help2+
                                              ' or Адрес like'+help2+
                                              ' or ИНН like'+help2);
    Open;
  end;
...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from Представители where Табельный_номер_представителя like'+help2+
                                              ' or Фамилия like'+help2+
                                              ' or Имя like'+help2+
                                              ' or Отчество like'+help2+
                                              ' or Паспортные_данные like'+help2);
    Open;
  end;
...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from Представители where ИД_Представителя like'+h2);
    Open;
  end;
  Form4.ShowModal;
  DBGrid2.SetFocus;
...
begin
  if DBGrid2.SelectedRows.Count > 0 then
    begin
      case MessageBox(Form2.Handle,'Удалить запись о представителе?',
    ...
    begin
      ...
      MessageBox(Form2.Handle,'Выберите прeдставителя!',
...
if DBGrid1.SelectedRows.Count > 0 then
    begin
      case MessageBox(Form2.Handle,'Удалить запись о партнере?',
        'Подтверждение',MB_YESNO or MB_ICONQUESTION) of
      IDYES:
        begin
    ...
    begin
      ...
      MessageBox(Form2.Handle,'Выберите партнера!',
        'Ошибка',MB_OK or MB_ICONERROR)
    end;
end;
...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from Предприятие_партнер where ИД_Предприятия like'+h2);
    Open;
  end;
  Form6.ShowModal;
  DBGrid1.SetFocus;
...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from Представители where ИД_Предприятия like'+h2);
    Open;
  end;
