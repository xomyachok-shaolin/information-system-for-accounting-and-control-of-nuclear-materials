...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from Тип_материала where название_материала like '+help2+
                                              ' or код_типа_материала like'+help2+
                                              ' or вес_каждого_элемента like'+help2);
    Open;
  end;
end;
...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from тм_уе where "Серийный № УЕ" like'+help2);
    Open;
  end;
...
begin
  ...
  case MessageBox(Form16.Handle,'Вы уверены, что хотите отправить учет. единицу?',
      ...
      IDYES:
        begin
          ...
          ADOStoredProc1.Parameters.Items[1].Value := Form29.id_inv;  // наклад

          Try
            ADOStoredProc1.ExecProc;
            ADOQueryAU.Requery;

            Application.MessageBox('Отправление материала зафиксировано', 'Транзакция выполнена', MB_OK or MB_ICONINFORMATION);
...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from тм_уе where ид_уе ='+h2);
    Open;
  end;
...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from тм_уе where тип_накладной = 2 and ИД_ПОМЕЩЕЩЕНИЯ is not null '+
      'and "ИД ТИПА МАТЕРИАЛА" ='+h2);
    Open;
  end;
