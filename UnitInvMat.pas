...
begin
  ...
  if DBGrid2.DataSource.DataSet.IsEmpty then
    begin
      MessageBox(Form30.Handle,'Запись не найдена!',
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from тм_уе where ИД_УЕ like'+h2);
    Open;
  end;
  ...
  Form33.DBLookupComboBox1.KeyValue :=
    Form33.ADOQuery2.FieldByName('ИД_ТИПА_КОНТЕЙНЕРА').AsInteger;

  Form33.ComboBox1.Text :=  Form33.ADOQuery2.FieldByName('Форма').AsString;

  Form33.Edit1.Text := Form33.ADOQuery2.FieldByName('Вес нетто').AsString;

  Form33.DBLookupComboBox2.KeyValue :=
    Form33.ADOQuery2.FieldByName('ИД_ВЕСОВ').AsInteger;
...
if DBGrid2.SelectedRows.Count > 0 then
    begin
      case MessageBox(Form30.Handle,'Удалить запись об УЕ?',
