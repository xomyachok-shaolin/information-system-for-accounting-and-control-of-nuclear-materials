...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from Должности where ИД_Должности like'+h2);
    Open;
  end;
  Form21.ShowModal;
  DBGrid2.SetFocus;
...
begin
      ...
      case MessageBox(Form18.Handle,'Удалить запись о должности?',
...
procedure TForm18.DBGrid2DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  if (Column.FieldName = 'ПРАВО_ОТВЕТСТВ_НАКЛАДНОЙ') or
    (Column.FieldName = 'ПРАВО_ОТВЕТСТВ_ПОМЕЩЕНИЯ') then // Модифицируйте под себя
...
begin
  ...
  begin
    close;
    SQL.Clear;
    SQL.Add('select * from должности where НАИМЕНОВАНИЕ like '+help2);
    Open;
  end;
end;
...
begin
  ...
  DBCheckBox1.DataField  := 'ПРАВО_ОТВЕТСТВ_НАКЛАДНОЙ';
  DBCheckBox1.Visible    := False;
  DBCheckBox1.Caption    := '';
